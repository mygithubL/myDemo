...
u8 OV7670_Init(void)
{
	...
	GPIO_InitStructure.GPIO_Pin  = FIFO_VSYNC; 	//PA8 输入 上拉
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
 	GPIO_Init(GPIO_VSYNC, &GPIO_InitStructure);
	GPIO_SetBits(GPIO_VSYNC,FIFO_VSYNC);
	
 	
	GPIO_InitStructure.GPIO_Pin = FIFO_WEN|FIFO_RCLK;				 // 端口配置
 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 //推挽输出
 	GPIO_Init(GPIO_RCLK, &GPIO_InitStructure);
 	GPIO_SetBits(GPIO_RCLK,GPIO_Pin_3|FIFO_RCLK);	

	
	GPIO_InitStructure.GPIO_Pin  = 0xff; //PC0~7 输入 上拉
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU;
 	GPIO_Init(GPIO_FIFO, &GPIO_InitStructure);
	 
	
  GPIO_InitStructure.GPIO_Pin  = FIFO_WRST;  
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
 	GPIO_Init(GPIO_WRST, &GPIO_InitStructure);
	GPIO_SetBits(GPIO_WRST,FIFO_WRST);
	
	GPIO_InitStructure.GPIO_Pin  = FIFO_RRST|FIFO_OE;  
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
 	GPIO_Init(GPIO_RRST, &GPIO_InitStructure);
	GPIO_SetBits(GPIO_RRST,FIFO_RRST|FIFO_OE);
