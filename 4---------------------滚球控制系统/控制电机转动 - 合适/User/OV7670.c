...
{
	I2C_Start();
	if(0==I2C_Send_Byte(OV7670_DEVICE_WRITE_ADDRESS))//CMOS器件地址（写）
	{
		I2C_Stop();
		return(0);
	}
	delay_us(100);
  	if(0==I2C_Send_Byte(regID)) //CMOS寄存器地址
	{
		I2C_Stop();
		return(0);
	}
	delay_us(100);
  	if(0==I2C_Send_Byte(regDat))//待写入指定寄存器的值
	{
		I2C_Stop();
		return(0);
	}
...
unsigned char rdCmos7670Reg(unsigned char regID)
{
	...
	I2C_Start();
	if(0==I2C_Send_Byte(OV7670_DEVICE_WRITE_ADDRESS))
	{
		I2C_Stop();
		return(0);
	}
	delay_us(100);
  	if(0==I2C_Send_Byte(regID))
	{
		I2C_Stop();
		return(0);
	}
	...
	I2C_Start();
	if(0==I2C_Send_Byte(OV7670_DEVICE_READ_ADDRESS))
	{
		I2C_Stop();
		return(0);
	}
