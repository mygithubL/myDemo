...
u16 goal_x,goal_y,goal_w,goal_h;//目标物坐标
  ...
  u8 angle;
	...
	RESULT Resu_zone1;
	RESULT Resu_zone2;
	TARGET_CONDI Condition_goal={0,160,0,160,230,255,20,20,320,240};	//  红色 hls的阈值，识别时用的
	TARGET_CONDI Condition_zone1={0,160,0,160,0,0,20,20,320,240};	  //  1区域黑色 hls的阈值，识别时用的
	TARGET_CONDI Condition_zone2={0,160,0,160,230,255,20,20,320,240};	  //  2区域白光 hls的阈值，识别时用的
  ...
  u16 color_H[2]={0,0}; 	
	if(ov_sta)//有帧中断更新？
	{
		...
		for(j=0;j<76800;j++)
		{
			...
			OV7670_RCK_L;
			color|=GPIOC->IDR&0XFF;	//读数据
			color_H[0]=GPIOC->IDR&0XFF;
			OV7670_RCK_H; 
			LCD->LCD_RAM=color;
      SPI_Flash_Write_Page((u8 *)color_H, 0x100000+j*2, 2);			
      			
		}  
 
     if(Trace_goal(&Condition_goal,&Resu_goal) )                      //API
			{		
				LCD_Fill(Resu_goal.x-Resu_goal.w/2,Resu_goal.y-Resu_goal.h/2,Resu_goal.x+Resu_goal.w/2,Resu_goal.y-Resu_goal.h/2+1,0xf800);//u16 x,u16 y,u16 width,u16 hight,u16 Color
				LCD_Fill(Resu_goal.x-Resu_goal.w/2,Resu_goal.y-Resu_goal.h/2,Resu_goal.x-Resu_goal.w/2+1,Resu_goal.y+Resu_goal.h/2,0xf800);
				LCD_Fill(Resu_goal.x-Resu_goal.w/2,Resu_goal.y+Resu_goal.h/2,Resu_goal.x+Resu_goal.w/2,Resu_goal.y+Resu_goal.h/2+1,0xf800);
				LCD_Fill(Resu_goal.x+Resu_goal.w/2,Resu_goal.y-Resu_goal.h/2,Resu_goal.x+Resu_goal.w/2+1,Resu_goal.y+Resu_goal.h/2,0xf800);
...
//				printf("goal_x=%d      goal_y=%d\r\n",Resu_goal.x,Resu_goal.y);
			...
			{		
				LCD_Fill(Resu_zone1.x-Resu_zone1.w/2,Resu_zone1.y-Resu_zone1.h/2,Resu_zone1.x+Resu_zone1.w/2,Resu_zone1.y-Resu_zone1.h/2+1,0xf800);//u16 x,u16 y,u16 width,u16 hight,u16 Color
				LCD_Fill(Resu_zone1.x-Resu_zone1.w/2,Resu_zone1.y-Resu_zone1.h/2,Resu_zone1.x-Resu_zone1.w/2+1,Resu_zone1.y+Resu_zone1.h/2,0xf800);
				LCD_Fill(Resu_zone1.x-Resu_zone1.w/2,Resu_zone1.y+Resu_zone1.h/2,Resu_zone1.x+Resu_zone1.w/2,Resu_zone1.y+Resu_zone1.h/2+1,0xf800);
				LCD_Fill(Resu_zone1.x+Resu_zone1.w/2,Resu_zone1.y-Resu_zone1.h/2,Resu_zone1.x+Resu_zone1.w/2+1,Resu_zone1.y+Resu_zone1.h/2,0xf800);
...
//				printf("zone_1_x=%d      zone_1_y=%d\r\n",zone_1_x,zone_1_y);
			...
			{		
				LCD_Fill(Resu_zone2.x-Resu_zone2.w/2,Resu_zone2.y-Resu_zone2.h/2,Resu_zone2.x+Resu_zone2.w/2,Resu_zone2.y-Resu_zone2.h/2+1,0);//u16 x,u16 y,u16 width,u16 hight,u16 Color
				LCD_Fill(Resu_zone2.x-Resu_zone2.w/2,Resu_zone2.y-Resu_zone2.h/2,Resu_zone2.x-Resu_zone2.w/2+1,Resu_zone2.y+Resu_zone2.h/2,0);
				LCD_Fill(Resu_zone2.x-Resu_zone2.w/2,Resu_zone2.y+Resu_zone2.h/2,Resu_zone2.x+Resu_zone2.w/2,Resu_zone2.y+Resu_zone2.h/2+1,0);
				LCD_Fill(Resu_zone2.x+Resu_zone2.w/2,Resu_zone2.y-Resu_zone2.h/2,Resu_zone2.x+Resu_zone2.w/2+1,Resu_zone2.y+Resu_zone2.h/2,0);
