...
xMBPortSerialGetByte( CHAR * pucByte )
{
     ...
     * by the protocol stack after pxMBFrameCBByteReceived( ) has been called.
	   返回UARTs接收缓冲区中的字节。这个函数被调用通过调用pxMBFrameCBByteReceived()后的协议栈。
     */
...
// 
//  u8 SBUF_byte=0;				  //定义接收1个字节的缓冲区

// if (USART_GetITStatus(USART2,USART_IT_RXNE)!=RESET)								  //判断接收是否完成
// 	{									  //如果接收完成
//		
//		SBUF_byte=USART_ReceiveData(USART2);					  //接收数据放入缓冲区
//		if((vb==0)&&(SBUF_byte!=0x01))	  //判断第1帧数据是否为0X01
//			{							  //如果第1帧数据不是0X01
//				point=0;				  //接收缓冲区当前位置清0
//				rx_ok=0;				  //接收完成位清0
//				vb=0;					  //接收数据标志位清0
...
//		if(vb==1)						   //如果接收标志位等于1
//			{
//				receive_byte[point++]=USART_ReceiveData(USART2);			 //把缓冲区数据放入接收字节数组中
//				if(point>=8)								 //如果接收的字节大于等于8个时
//				{											 //MODBUS主机发送的字节数为8个字节
//					rx_ok=1;								 //接收完成标志位置1
//					point=0;								 //接收缓冲区当前位置清0
//					vb=0;									 //接收数据标准位清0
