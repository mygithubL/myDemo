...
#define REG_INPUT_NREGS    1       //输入寄存器数量//保持寄存器起始地址
  

#define REG_HOLDING_START  0x0000  //保持寄存器起始地址
#define REG_HOLDING_NREGS  1       //保持寄存器数量
...
//输入寄存器内容
uint16_t usRegInputBuf[REG_INPUT_NREGS] = {0x0001};
//寄存器起始地址
uint16_t usRegInputStart =0000;

//保持寄存器内容
uint16_t usRegHoldingBuf[REG_HOLDING_NREGS] = {0x1000};
//保持寄存器起始地址
...
extern u8 str2;
extern int flag;
const UCHAR buf[6] = {01,02,00,00,00,01};
UCHAR bz=1;
			   ...
			   if(GPIO_ReadInputDataBit(GPIOC,GPIO_Pin_5) == Bit_RESET)
					{
						 ...
						 eMBRTUSend(bz, buf, (unsigned short)6 );
