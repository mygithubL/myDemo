...
int main(void) 
{
	 ...
	 eMBInit(MB_RTU, 0x02, 0x02, 9600, MB_PAR_NONE); //初始化 RTU模式 从机地址为2 USART29600 无校验 
	 ...
	 if(GPIO_ReadInputDataBit(GPIOC,GPIO_Pin_2) == Bit_RESET)
  {
	   ...
	   if(GPIO_ReadInputDataBit(GPIOC,GPIO_Pin_2) == Bit_RESET)
	  {
       ...
       while(USART_GetFlagStatus(USART2,USART_FLAG_TXE)==RESET);
		   USART_SendData(USART2, 0x5);	//发送字符N
	     while(USART_GetFlagStatus(USART2,USART_FLAG_TXE)==RESET);
		   delay_ms(2);
...
{	
	...
	/*PA8按键参数设置*/			
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_1|GPIO_Pin_2;	/*选择要用的GPIO引脚*/					 
